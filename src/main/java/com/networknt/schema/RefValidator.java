...
public class RefValidator extends BaseJsonValidator implements JsonValidator {
    ...
    public RefValidator(String schemaPath, JsonNode schemaNode, JsonSchema parentSchema, ValidationContext validationContext) {
        ...
        JsonSchema schema = resolveRef(schemaNode, parentSchema, validationContext);
        if (schema == null) {
            throw new JsonSchemaException(ValidationMessage.of(ValidatorTypeCode.REF.getValue(), CustomErrorMessageType.of("internal.unresolvedRef", new MessageFormat("{0}: Reference {1} cannot be resolved")), schemaPath, schemaNode.asText()));
        }
    }

    public static JsonSchema resolveRef(JsonNode schemaNode, JsonSchema parentSchema, ValidationContext validationContext) {
